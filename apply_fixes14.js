...
`;
  ...
  const oldPostSection = `    debugEvent.adapter_response_detail = JSON.stringify(normalizedResponse, null, 2);
  ...
  const newPostSection = `    debugEvent.adapter_response_detail = JSON.stringify(normalizedResponse, null, 2);
  ...
  if (code.includes(oldPostSection)) {
    ...
    console.log("Could not find post block in server.js!");
  }
}
