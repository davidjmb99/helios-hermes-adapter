...
const newStreamFunc = \sync function consumeHermesStream(streamId) {
  if (!hermesCookie) {
    await hermesLogin();
  }

  const url = \\\\\\/api/chat/stream?stream_id=\\\\\\;
  ...
  if (!response.ok) {
    ...
    throw new Error(\\\Hermes stream connection failed HTTP \\\: \\\\\\);
  }
  ...
  const processEvent = (eventName, dataStr) => {
    ...
    if (evName === "assistant.delta" || evName === "token") {
      ...
      throw new Error(\\\Hermes stream reported error: \\\\\\);
...
const newNormFunc = \unction normalizeAdapterResponse(result) {
