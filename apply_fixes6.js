...
function applyFixes() {
  ...
  // 1. Health endpoint
  const healthTarget = `app.get("/health", (req, res) => {
    res.json({
      ok: true,
  ...
  const healthReplacement = `app.get("/health", (req, res) => {
    res.json({
      ...
      token_estimation_enabled: TOKEN_ESTIMATION_ENABLED,`;
  code = code.replace(healthTarget, healthReplacement);
  ...
  // Try the older version regex if rawReply wasn't present
  const streamRegexOld = /async function consumeHermesStream\(streamId\) \{[\s\S]*?return \{ answer: accumulatedAnswer, firstTokenTime \};\n\}/;
  code = code.replace(streamRegexOld, newStreamFunc);

  // 3. app.post
  // Insert processingStage at the beginning of app.post
  const appPostTarget = `app.post("/helios/message", async (req, res) => {`;
  const appPostReplacement = `app.post("/helios/message", async (req, res) => {
  let processingStage = "request_received";
  let requestPhone = null;
  let requestPatientDisplayName = "Contacto sin identificar";`;
  code = code.replace(appPostTarget, appPostReplacement);

  const telemetryCtxTarget = `const telemetryCtx = await startAdapterEvent(req.body || {});`;
  const telemetryCtxReplacement = `processingStage = "telemetry_started";
  const telemetryCtx = await startAdapterEvent(req.body || {});`;
  code = code.replace(telemetryCtxTarget, telemetryCtxReplacement);

  const normalizedTarget = `  try {
    normalized = normalizeGatewayPayload(payload);
  } catch (err) {
    normalized = { raw: payload };
  }`;
  const normalizedReplacement = `  try {
    normalized = normalizeGatewayPayload(payload);
    processingStage = "session_resolved";
  ...
  code = code.replace(normalizedTarget, normalizedReplacement);

  const startHermesTarget = `result = await sendMessageToHermes(payload);`;
  const startHermesReplacement = `processingStage = "message_sent";
      result = await sendMessageToHermes(payload);`;
  code = code.replace(startHermesTarget, startHermesReplacement);

  const extractSessionTarget = `sessionId = result.sessionId || "";
    streamId = result.streamId || "";`;
  const extractSessionReplacement = `processingStage = "assistant_completed_received";
    sessionId = result.sessionId || "";
    streamId = result.streamId || "";`;
  code = code.replace(extractSessionTarget, extractSessionReplacement);

  const normalizeResponseTarget = `const normalizedResponse = normalizeAdapterResponse(result);`;
  const normalizeResponseReplacement = `processingStage = "contract_parsing";
    const normalizedResponse = normalizeAdapterResponse(result);
    processingStage = "contract_validated";`;
  code = code.replace(normalizeResponseTarget, normalizeResponseReplacement);

  const finishTelemetryTarget = `await finishAdapterEvent(
      telemetryCtx,`;
  const finishTelemetryReplacement = `processingStage = "telemetry_finishing";
    await finishAdapterEvent(
      telemetryCtx,`;
  code = code.replace(finishTelemetryTarget, finishTelemetryReplacement);

  const returnResTarget = `return res.json(normalizedResponse);`;
  const returnResReplacement = `processingStage = "response_returned";
    return res.json(normalizedResponse);`;
  code = code.replace(returnResTarget, returnResReplacement);

  const catchBlockTarget = `} catch (error) {
    console.error("Adapter error:", error);`;
  const catchBlockReplacement = `} catch (error) {
  ...
  const finishCtxTarget = `hermes_first_token_ms: typeof hermesFirstTokenMs !== 'undefined' ? hermesFirstTokenMs : null,`;
  const finishCtxReplacement = `hermes_first_token_ms: typeof hermesFirstTokenMs !== 'undefined' ? hermesFirstTokenMs : null,
          session_id: sessionId,
          stream_id: streamId,
          processing_stage: processingStage,`;
  code = code.replace(new RegExp(finishCtxTarget.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&'), 'g'), finishCtxReplacement);

  // Add error normalization rule
  const errNormTarget = `const normalizedError = normalizeProviderError(error);`;
  const errNormReplacement = `let normalizedError = normalizeProviderError(error);
  ...
  code = code.replace(errNormTarget, errNormReplacement);
